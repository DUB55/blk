...
const onlinecountws = new WebSocket(window.location.protocol.replace('http', 'ws') + '//' + window.location.host + '/onlinecount');
...
`
  ...
  let ws = new WebSocket(window.location.protocol.replace('http', 'ws') + '//' + window.location.host + '/chat');
  ws.onopen = () => {
  ...
  function discordConnect() {
    if (ws.readyState == 3) {
      ws = new WebSocket(window.location.protocol.replace('http', 'ws') + '//' + window.location.host + '/chat');
    }
    ws.onmessage = (m) => {
      try {
        handleWs(m.data);
